...
namespace Proxima
{
	...
	const std::filesystem::path FileSystem::PLAYER_DATA_FILENAME = BASE_PATH / "PlayerData.txt";
	...
	bool FileSystem::ReadStats(StatsCategory channel, OUT std::map<std::string, float>& out)
	{
		CreateStatsFolder();
	...
	bool FileSystem::WriteStats(StatsCategory channel, const std::map<std::string, float>& in)
	{
		CreateStatsFolder();
	...
	bool FileSystem::ReadProfile(OUT Profile& name)
	{
		const std::string pData = ReadFileInternal(PLAYER_DATA_FILENAME);
		...
		std::istringstream f(pData);
		{
			...
			while (std::getline(f, line)) {
				...
				if (pos != std::string::npos)
				{
					...
					if (key == "PreferredHostingPort")
					{
						name.hostPort = static_cast<uint16_t>(std::stoi(value));
					}

					if (key == "SteamID")
					{
						name.steamID.m_unAll64Bits = static_cast<uint64_t>(std::stoull(value));
					}
				}
			}
		}
	...
	bool FileSystem::WriteProfile(const Profile& profile)
	{
		const std::string data = std::format("PlayerName={}\nPreferredHostingPort={}\nSteamID={}\n", profile.name, profile.hostPort, profile.steamID.m_unAll64Bits);
		return WriteFileInternal(data, PLAYER_DATA_FILENAME);
	}
}