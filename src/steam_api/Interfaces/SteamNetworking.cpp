...
namespace Steam
{
	bool Networking::SendP2PPacket(CSteamID steamIDRemote, const void* pubData, uint32 cubData, EP2PSend eP2PSendType)
	{
		DUMP_FUNC_NAME();

		return false;
	}
	bool Networking::IsP2PPacketAvailable(uint32* pcubMsgSize)
	{
		return false;
	}
	bool Networking::ReadP2PPacket(void* pubDest, uint32 cubDest, uint32* pcubMsgSize, CSteamID* psteamIDRemote)
	{
		DUMP_FUNC_NAME();
